<x-app-layout>
    ...
    <div class="bg-white dark:bg-gray-700 rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="p-6">
            ...
            @auth
                @if (Auth::user()->is_admin)
                    <form action="{{ route('activities.destroy', $activity->id) }}" method="POST" class="mt-4">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="bg-red-600 text-white font-medium py-2 px-4 rounded shadow hover:bg-red-700">
                            Verwijderen
                        </button>
                    </form>
                @endif
